 ...
 */
 ...
 class HashingProblems {
   ...
   public int twoSums(int[] numbers, int k) {
        ...
        int count = 0;
        //POPULATE AND PROBE AT THE SAME TIME = O(n) AKA IM TOO GOOD
        for (int num : numbers) {
            //checks if (num-k) is in the set which would mean num-k+k = num
            if (set.contains(num - k)) {
                count++;
            }
            //adds num to the set after initial check so other nums can use it
            set.add(num);
        }
        return count;
    } 
